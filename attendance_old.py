...
class Attendance:
    ...
    def __init__(self, root):
        ...
        # self.engine.say('You are now in Attendance Management System')
        ...
        # title section
        title_lb1 = Label(bg_img, text="Attendance Pannel", font=(
        ...
        # Student id
        studentId_label = Label(left_frame, text="Student ID:", font=(
        ...
        # Studnet Name
        student_name_label = Label(left_frame, text="Name:", font=(
            "verdana", 12, "bold"), fg="navyblue", bg="white")
        ...
        # Right Label Frame
        right_frame = LabelFrame(main_frame, bd=2, bg="white", relief=RIDGE, text="Records", font=(
            "verdana", 12, "bold"), fg="navyblue")
        right_frame.place(x=680, y=10, width=560, height=480)

        # -----------------------------Table Frame-------------------------------------------------
        # Table Frame
        # Searching System in Right Label Frame
        table_frame = Frame(right_frame, bd=2, bg="white", relief=RIDGE)
        table_frame.place(x=10, y=90, width=550, height=360)
    ...
    # Update button
        del_btn = Button(right_frame, command=self.update_data, text="Update", width=12, font=(
            "verdana", 12, "bold"), fg="white", bg="navyblue")
        del_btn.grid(row=1, column=2, padx=6, pady=10, sticky=W)
    # Update button
        del_btn = Button(right_frame, command=self.delete_data, text="Delete", width=12, font=(
            "verdana", 12, "bold"), fg="white", bg="navyblue")
        del_btn.grid(row=1, column=4, padx=6, pady=10, sticky=W)
    ...
    def update_data(self):
        if self.var_id.get() == "" or self.var_roll.get == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
            self.engine.say('Please Fill All Fields are Required')
            self.engine.runAndWait()
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update attendance_test set std_id=%s,std_roll_no=%s,std_name=%s,std_time=%s,std_date=%s,std_attendance=%s where std_id=%s", (
                        self.var_id.get(),
                        self.var_roll.get(),
                        self.var_name.get(),
                        self.var_time.get(),
                    ...
                    if not Update:
                        return
                self.engine.say('Successfully Updated')
                self.engine.runAndWait()
                messagebox.showinfo(
    ...
    def delete_data(self):
        if self.var_id.get() == "":
            ...
            try:
                self.engine.say('please Confirm,Do you really want to Delete')
                self.engine.runAndWait()
                ...
                if delete > 0:
                    conn = mysql.connector.connect(
                        username='root', password='vishh@123', host='localhost', database='attendence_management')
                    mycursor = conn.cursor()
                    sql = "delete from attendance_test where std_id=%s"
                    val = (self.var_id.get(),)
                ...
                self.fetch_data()
                conn.close()
                self.engine.say('Successfully Deleted')
                self.engine.runAndWait()
                messagebox.showinfo(
    ...
    def fetch_data(self):
        ...
        mycursor.execute("select * from attendance_test")
    ...
    def exportCsv(self):
        try:
            ...
            with open(fln, mode="w", newline="") as myfile:
                ...
                self.engine.say('Export Data Successfully')
                self.engine.runAndWait()
        ...
        if self.var_id.get() == "" or self.var_roll.get == "" or self.var_name.get() == "" or self.var_time.get() == "" or self.var_date.get() == "" or self.var_attend.get() == "Status":
            ...
            try:
                conn = mysql.connector.connect(
                    username='root', password='vishh@123', host='localhost', database='attendence_management')
                mycursor = conn.cursor()
                mycursor.execute("insert into attendance_test values(%s,%s,%s,%s,%s,%s)", (
                    self.var_id.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
                ...
                self.fetch_data()
                conn.close()
                self.engine.say('All Records are Saved in Database')
                self.engine.runAndWait()
                messagebox.showinfo(
...
#     CREATE TABLE `attendence_management`.`attendance_record` (
...
# CREATE TABLE `attendence_management`.`attendance_test` (
#   `std_id` INT NOT NULL,
#   `std_roll_no` VARCHAR(45) NULL,
#   `std_name` VARCHAR(45) NULL,
#   `std_time` VARCHAR(45) NULL,
#   `std_date` VARCHAR(45) NULL,
#   `std_attendance` VARCHAR(45) NULL,
#   PRIMARY KEY (`std_id`));
